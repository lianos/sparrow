...
abstract: >
  ...
  sparrow.shiny package. This package also provides an easy wrapper to single
...
interactively explore `SparrowResult` objects. The application can be invoked
as follows:
...
using a little `reshape2` mojo and the `sparrow::corplot` function.
...
TODO: explore this more deeply, but the examples in ?scoreSingleSamples suggests
that to be the case in that example.
...
<!--
Update MSigDb Section ==========================================================
-->

TODO: This needs update to reflect our use of

The sparrow package provides the `getMSigGeneSetDb` utility function to provide
...
```{r, eval=FALSE}
mg <- seas(gdb, vm, vm$design, cm[, 'BvH'],
                methods=c('goseq'),
                treat.lfc=log2(1.5),
...
```{r, eval=FALSE}
mgx <- seas(gdb, vm, vm$design, cm[, 'BvH'],

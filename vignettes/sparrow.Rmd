...
* A sister `sparrow.shiny` package provides an `explore` function, which is
  invoked on `SparrowResult` objects returned from a call to `seas`.
...
* A convenience `getMSigGeneSetDb()` function, which enables the user to
  fetch arbitraty subsets of the MSigDB collections mapped to different
  organisms and identifier types.

<div class="note">
The [BiocSet][] package was introduced in Bioconductor version 3.10, which
should supersede the `GeneSetDb` class introduced here. sparrow will endeavor
to transition to using that class to store gene set collections in the near
future.
</div>
...
All GSEA methods require the use of a `GeneSetDb` object, which is a new class
...
dplyr::select(sparrow::sparrow_methods(),
...
The *sparrow* package provides a `GeneSetDb` class, which is used instead of
...
Note that when running a "goseq" analysis, sparrow will (by default) run
...
```{r iplot-wnt-beta-gsea, fig.asp=1, message=FALSE, warning=FALSE}
iplot(mg, 'H', 'HALLMARK_WNT_BETA_CATENIN_SIGNALING',
...
A sister `sparrow.shiny` package is available that can be used to
interactively explore `SparrowResult` objects to help you try to make sense
...
**NOTE:** The GeneSetDb class was initially developed to address some
peculiarities of storing, querying, and manipulating gene sets using the
old-school `GSEABase::GeneSetCollection` class. It is not meant to replace the
more recently developd BiocSet class, and we plan to support and migrate to that
gene set container in the future. This section provides a quick overview of
some of the functionality of gene set container.
...
The sparrow package provides convenience funcitons to fetch genesets from many
...
Let's say we wanted to include only the genesets where their "subcategory" was
"[CP:PID][msigdbpid]", we would like to do it like this:

...
* A sister `multiGSEA.shiny` package provides an `explore` function, which is
  invoked on `SparrowResult` objects returned from a call to `multiGSEA`.
...
should supersede the `GeneSetDb` class introduced here. multiGSEA will endeavor
...
```{r multiGSEA-methods, echo=FALSE, message=FALSE, warning=FALSE}
dplyr::select(multiGSEA::sparrow_methods(),
...
* [MSigDB][msigdb] via `getMSigGeneSetDb(...)`. Although the core `multiGSEA`
  ...
  install the `msigdb.data` package to include the data and utility functions
  for the identifier and species mapping.
...
[msigdbr](https://cran.r-project.org/package=msigdbr) within the
[msigdb.data](https://github.com/lianos/msigdb.data) package.

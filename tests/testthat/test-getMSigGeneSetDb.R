...
test_that("url function stored correctly", {
  ...
  gdb.npro <- getMSigGeneSetDb("C5", promote_subcategory_to_collection = FALSE)
  ...
  for (gocat in names(genesets)) {
    ...
    expect_equal(pro.url, expectec.url,
                 info = paste("promoted", gocat, goname, sep = ":"))
    npro.url <- geneSetURL(gdb.pro, sprintf("GO:%s", gocat), goname)
    expect_equal(pro.url, expectec.url,
                 info = paste("nopromo", gocat, goname, sep = ":"))
  }

  go.df <- msigdb.data::msigdb_retrieve("C5", "human")
